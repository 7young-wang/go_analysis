 ...
 sampling_procedure<-function(args)
 {
  ...
  if (args[1] == "BP") {
    transp<-"./supporting_files/target_matrix_BP.RData"
    cat("This is BP analysis \n")
  } else if (args[1] == "MF") {
    transp<-"./supporting_files/target_matrix_MF.RData"
    cat("This is MF analysis \n")
  } else if (args[1] == "CC") {
    transp<-"./supporting_files/target_matrix_CC.RData"
    cat("This is CC analysis \n")
  }
  ...
  cat(paste(length(unique(gene$genes)),"genes from",length(region),"loci were found with propagation probability...\n"))
  ...
  if(is.na(args[2])) res_path<-"./iRIGS_result/"     else res_path<-args[2]
  ...
  if(is.na(args[3])) res_file<-strsplit(args[1],"/")[[1]][length(strsplit(args[1],"/")[[1]])]  else res_file<-args[3]
